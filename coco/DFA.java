...
//    if (ch < ' ' || ch >= 127 || ch == '\'' || ch == '\\') {
//      return "String.fromCharCode(" + Integer.toString((int)ch) + ")";
//    }
//    else return "'" + ch + "'";
  ...
  private void PutRange(CharSet s) {
  	for (CharSet.Range r = s.head; r != null; r = r.next) {
    	...
    	else if (r.from == 0) { gen.print("ch <= " + ((int) r.to)); }
    	else { gen.print("ch >= " + ((int) r.from) + " && ch <= " + ((int) r.to)); }
  ...
  void GenComment(Comment com, int i) {
    gen.println();
    gen.print  ("\tfunction Comment" + i + "():Bool "); gen.println("{");
    gen.println("\t\tvar level:Int = 1, pos0:Int = pos, line0:Int = line, col0:Int = col, nch:Int = buffer.Peek();");
    if (com.start.length() == 1) {
      gen.println("\t\tNextCh();");
      GenComBody(com);
    } else {
//      gen.println("\t\tNextCh();");
      gen.print  ("\t\tif (n" + ChCond(com.start.charAt(1)) + ") "); gen.println("{");
      gen.println("\t\t\tNextCh(); NextCh();");
      GenComBody(com);
//      gen.println("\t\t} else {");
//    gen.println("\t\t\tbuffer.setPos(pos0); NextCh(); line = line0; col = col0;");
    ...
    for (int i = 0; i < ts.length; ++i) {
      ...
      while (iter.hasNext()) {
        ...
        if (sym.tokenKind == Symbol.litToken) {
          ...
          gen.println("\t\tliterals.set(" + name + ", " + sym.n + ");");
        }
      }
    }
  }
  ...
  void WriteState(State state) {
    ...
    if (endOf == null) {
      gen.println("t.kind = noSym; break;}");
    } else {
      gen.print("t.kind = " + endOf.n + "; ");
      if (endOf.tokenKind == Symbol.classLitToken) {
        gen.println("t.val = tval.toString(); tval = new StringBuf(); CheckLiteral(); return t;}");
      } else {
        gen.println("break;}");
      }
    }
  ...
  void WriteStartTab() {
    for (Action action = firstState.firstAction; action != null; action = action.next) {
      ...
      if (action.typ == Node.chr) {
        ...
        for (CharSet.Range r = s.head; r != null; r = r.next) {
	  if (r.from == r.to) gen.println("\t\tstart.set(" + r.from + ", " + targetState + ");");
          else gen.println("\t\tfor (i in " + r.from + "..." + (r.to + 1) + ") start.set(i, " + targetState + ");");
        }
      }
    }
  ...
  void OpenGen(boolean backUp) {
    try {
      File f = new File(tab.outDir, "Scanner.hx");
      if (backUp && f.exists()) {
  ...
  public void WriteScanner() {
    ...
    gen.println("\tstatic var maxT:Int = " + (tab.terminals.size() - 1) + ";");
    gen.println("\tstatic var noSym:Int = " + tab.noSym.n + ";");
    if (ignoreCase)
      gen.print("\tvar valCh:String;       // current input character (for token.val)");
    ...
    if (ignoreCase) {
      ...
      gen.println("\t\t\tvalCh = String.fromCharCode(ch);");
      gen.println("\t\t\tch = tvalCh.toLowerCase();");
      gen.println("\t\t}");
    }
    CopyFramePart("-->casing2");
    if (ignoreCase) gen.println("\t\t\ttval.addChar(valCh); ");
    else gen.println("\t\t\ttval.addChar(ch); ");
