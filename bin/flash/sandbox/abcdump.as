 ...
 *				 Iteratif - Olivier Bugalotto <contact@iteratif.net>
 * 
...
package abcdump
{
	...
	class MethodInfo extends MemberInfo
	{
		...
		function dump(abc:Abc, indent:String, attr:String="")
		{
			...
			if (code)
			{
				...
				sb.append(indent+"// local_count="+local_count+
					  " max_scope=" + max_scope +
					  " max_stack=" + max_stack +
	...
	class SlotInfo extends MemberInfo
	{
		...
		function dump(abc:Abc, indent:String, attr:String="")
		{
			...
			sb.append(indent+attr+def+" "+name+" extends "+it.base+"\n")
			var oldindent = indent
			indent += TAB
			if (it.interfaces.length > 0)
				sb.append(indent+"implements "+it.interfaces+"\n")
			sb.append(oldindent+"{\n")
	...
	class Abc
	{
		...
		function Abc(data:ByteArray, output:StringBuilder)
		{
			data.position = 0
			this.output = output;
			this.data = data
		...
		function parseCpool()
		{
			...
			sb.append("Cpool namespaces count "+ n +" size "+(data.position-start)+" "+int(100*(data.position-start)/data.length)+" %\n")
			start = data.position
			...
			sb.append("Cpool nssets count "+ n +" size "+(data.position-start)+" "+int(100*(data.position-start)/data.length)+" %\n")
			start = data.position
			...
			sb.append("Cpool names count "+ n +" size "+(data.position-start)+" "+int(100*(data.position-start)/data.length)+" %\n")
			start = data.position
		...
		function parseMethodInfos()
		{
			...
			for (var i:int=0; i < method_count; i++)
			{
				...
				if (m.flags & HAS_OPTIONAL)
				{
					...
					for( var k:int = param_count-optional_count; k < param_count; ++k)
					{
						...
						{
							if (!defaults[kind])
								sb.append("ERROR kind="+kind+" method_id " + i+"\n")
                            else
			...
			sb.append("MethodInfo count " +method_count+ " size "+(data.position-start)+" "+int(100*(data.position-start)/data.length)+" %\n")
		}
		...
		function parseInstanceInfos()
		{
			...
			sb.append("InstanceInfo size "+(data.position-start)+" "+int(100*(data.position-start)/data.length)+" %\n")
		}
		...
		function parseTraits(t:Traits)
		{
			...
			for (var i:int=0; i < namecount; i++)
			{
				...
				if (!member)
					sb.append("error trait kind "+kind+"\n")
				member.kind = kind
				member.name = name
		...
		function parseClassInfos()
		{
			...
			sb.append("ClassInfo size "+(data.position-start)+" "+int(100*(data.position-start)/data.length)+"%\n")
		}
		...
		function parseScriptInfos()
		{
			...
			sb.append("ScriptInfo size "+(data.position-start)+" "+int(100*(data.position-start)/data.length)+" %\n")
		}
		...
		function parseMethodBodies()
		{
			...
			sb.append("MethodBodies size "+(data.position-start)+" "+int(100*(data.position-start)/data.length)+" %\n")
		}
		...
		function dump(indent:String="")
		{
			...
			sb.append("OPCODE\tSIZE\t% OF "+totalSize+"\n")
			var done = []
			for (;;)
			{
				var max:int = -1;
				...
				sb.append(opNames[max]+"\t"+int(opSizes[max])+"\t"+int(100*opSizes[max]/totalSize)+"%\n")
			}
		}
	}
	...
	class Swf
	{
		...
		function Swf(data:ByteArray, output:StringBuilder)
		{
			this.data = data
			this.output = output;
			sb.append("size "+decodeRect()+"\n")
			sb.append("frame rate "+(data.readUnsignedByte()<<8|data.readUnsignedByte())+"\n")
			sb.append("frame count "+data.readUnsignedShort()+"\n")
			decodeTags()			
		}
		...
		private function decodeTags()
		{
	        ...
	        while (data.position < data.length)
	        {
				...
				sb.append(tagNames[type]+" "+length+"b "+int(100*length/data.length)+"%\n")
				switch (type)
				{
				case 0: return
				case stagDoABC2:
	...
	if( System.argv.length < 1 ) {
		...
		for each (var file in System.argv)
		{
			...
			case 67|87<<8|83<<16|6<<24: // SWC6
				...
				udata.uncompress()
				sb.append("decompressed swf "+csize+" -> "+udata.length+"\n")
				udata.position = 0
				new Swf(udata,sb)
				break
			...
			case 70|87<<8|83<<16|4<<24: // SWC4
				...
				break
			default:
				sb.append('unknown format '+version+"\n")
				break
			}
			
			var total = getTimer() - start;
			File.write(file+".il",sb.toString());
			print("File created... " + total + "ms");
		}
	}
}
